...
const X_BEARER_TOKEN = process.env.X_BEARER_TOKEN || "AAAAAAAAAAAAAAAAAAAAAFnD%2BAEAAAAAv8pNddMt0rrrya3%2F%2FsX2sIK%2B2Oc%3DsCKDSGaPCPJIK5EweMy2WtCeCiQckraToG0FCkb0BZCbsFgbSf";
...
function buildNewsQuery({topic, language = "es", onlyLinks = true, excludeRetweets = true}) {
    const parts = [];
    parts.push(`(${topic})`);
    if (language && language !== "any") parts.push(`lang:${language}`);
    if (excludeRetweets) parts.push("-is:retweet");
    if (onlyLinks) parts.push("has:links");
...
function formatTweets(tweets) {
    if (!tweets.length) return "No se encontraron posts recientes para esa búsqueda.";
...
server.tool(
    ...
    async ({topic, language, max_results, only_links, exclude_retweets}) => {
        try {
            ...
            return {
                content: [{type: "text", text: `Query usada: ${query}\n\n${formatTweets(tweets)}`}]
            };
        } catch (error) {
            return {content: [{type: "text", text: `Error consultando X: ${error.message}`}]};
        }
    }
);
...
server.tool(
    ...
    async ({topic, language, max_results}) => {
        try {
            const breakingTerms = language === "en"
            ...
            const query = cleanQuery(`(${topic}) ${breakingTerms}${lang} -is:retweet`);
            const tweets = await xRecentSearch(query, max_results);
            return {
                content: [{type: "text", text: `Query usada: ${query}\n\n${formatTweets(tweets)}`}]
            };
...
server.tool(
    ...
    async ({accounts, keyword, max_results}) => {
        try {
            const fromClause = accounts.map((a) => `from:${a.replace("@", "")}`).join(" OR ");
            const keywordClause = keyword ? ` (${keyword})` : "";
            const query = cleanQuery(`(${fromClause})${keywordClause} -is:retweet`);
            const tweets = await xRecentSearch(query, max_results);
            return {
                content: [{type: "text", text: `Query usada: ${query}\n\n${formatTweets(tweets)}`}]
            };
...
function createMcpServer() {
    ...
    server.tool(
        ...
        "Busca posts recientes en X/Twitter sobre un tema, optimizado para extraer noticias con enlaces.",
        {
            ...
            language: z.enum(["es", "en", "any"]).default("es"),
            max_results: z.number().int().min(10).max(100).default(10),
            only_links: z.boolean().default(true),
            exclude_retweets: z.boolean().default(true)
        },
        async ({topic, language, max_results, only_links, exclude_retweets}) => {
            try {
                ...
                return {
                    content: [{type: "text", text: `Error consultando X: ${error.message}`}]
                };
            }
        }
    );

    // registra aquí también breaking_x_news y x_news_from_accounts
...
app.listen(PORT, () => {
    console.log(`X News MCP running on port ${PORT}`);
});
